...
DEFINE_string(start_cluster_colors, "",
              "the initial colors for each cluster, as comma-separated hex");
DEFINE_string(final_cluster_colors, "",
              "the final colors for each cluster, as comma-separated hex");
...
int main(int argc, char **argv)
{
  ...
  vector<string> start_cluster_colors;
  vector<string> final_cluster_colors;

  if (FLAGS_start_cluster_colors != "")
  {
    SplitString(FLAGS_start_cluster_colors, ',', &start_cluster_colors);
    if (start_cluster_colors.size() != FLAGS_clusters)
    {
      LOG(FATAL) << start_cluster_colors.size() << " colors for "
                 << FLAGS_clusters << " clusters.";
    }
  }

  if (FLAGS_final_cluster_colors != "")
  {
    SplitString(FLAGS_final_cluster_colors, ',', &final_cluster_colors);
    if (final_cluster_colors.size() != FLAGS_clusters)
    {
      LOG(FATAL) << final_cluster_colors.size() << " colors for "
                 << FLAGS_clusters << " clusters.";
    }
  }
  ...
  for (int i = 0; i < FLAGS_clusters; ++i)
  {
    ...
    {
      ParseColor(start_cluster_colors[i], &c.red, &c.green, &c.blue);
    }

    if (FLAGS_final_cluster_colors != "")
    {
      ParseColor(final_cluster_colors[i], &c.final_red, &c.final_green, &c.final_blue);
    }
      ...
      for (int y = 0; y < height; ++y)
      {
        ...
        for (int x = 0; x < width; ++x)
        {
          ...
          if (pass == FLAGS_passes - 1)
          {
            ...
            if (FLAGS_final_cluster_colors != "")
            {
              PixelSetRed(pixels[x], clusters[closest].final_red);
              PixelSetGreen(pixels[x], clusters[closest].final_green);
              PixelSetBlue(pixels[x], clusters[closest].final_blue);
            }
            else
            {
  ...
  for (int c = 0; c < clusters.size(); ++c)
  {
    ...
    */
    ...
    printf("%02x%02x%02x",
           static_cast<int>(clusters[c].red * 255),
           static_cast<int>(clusters[c].green * 255),
           static_cast<int>(clusters[c].blue * 255));
  }
